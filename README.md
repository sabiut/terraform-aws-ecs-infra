...
ECS reads `DB_USER` and `DB_PASSWORD` from Secrets Manager only when a task starts, so on its own a rotation would leave running backend tasks with a password that no longer works. Secrets Manager publishes a `Secret Label Updated` event whenever the secret's `AWSCURRENT` label moves to a new version, which is what a rotation does. An EventBridge rule matches that event for the database secret and starts a small Step Functions state machine that forces a new deployment of both backend services and waits for each rollout to finish. ECS starts tasks with the new password before stopping the old ones; the inactive colour has no tasks, so nothing happens there. The state machine records the deployment it started and only succeeds once that deployment is the service's only one and reports `COMPLETED`, meaning every task started before the rotation is gone. An ECS rollback does not create a new deployment; it revives the previous one with its old tasks, so the state machine fails whenever the primary deployment is no longer the one it started, or that deployment reports `FAILED`. It times out if the rollout has not settled within an hour. A pipeline deployment that supersedes the redeploy during those minutes also fails the execution, even though its tasks are fresh; treat the alarm as a prompt to check that no backend task predates the rotation.
